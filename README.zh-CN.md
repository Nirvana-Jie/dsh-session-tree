...
| 尝试风险较高的方案 | 从所选会话的最近稳定回合一键创建分支，并在标题中保留完整祖先路径 |
...
- **原生导航。** 选择任意节点，通过 DSH 打开准确对应的会话；新创建的分支默认进入“对话”。
- **安全分叉。** 通过 DSH 原生 fork 操作从最近稳定回合创建子会话，并直接进入新分支继续工作。
- **保留谱系的标题。** 每个普通分支都会在父标题后追加同级局部序号：`任务 → 任务 (1) → 任务 (1) (1)`；同一父会话继续分叉时只递增最后一级序号。
...
4. 选择 **打开会话**，切换到准确对应的 DSH 会话；也可以选择 **从最近稳定回合创建分支**，创建带谱系标题的分支并进入新会话的“对话”视图。
...
该包同时是 DSH bundle 和浏览器客户端插件。bundle 层负责在 `web` profile 中启用插件；客户端注册 `conversation.view` 扩展，并从 DSH 获取会话存储、导航、fork、rename 操作和 UI primitives。插件根据所选父会话及其直接普通分支计算新分支的谱系标题，再交由 DSH 持久化。视图只投影会话元数据；会话持久化、修改以及当前“对话”或“轨迹”视图仍由 DSH 负责。
...
提交信息遵循 [Conventional Commits](https://www.conventionalcommits.org/) 规范，可以使用 `git cz` 生成，例如 `fix(client): use supported session navigation`。
