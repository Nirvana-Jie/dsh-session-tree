...
4. 选择 **打开会话**，切换到准确对应的 DSH 会话；也可以选择 **从最近稳定回合创建分支**，新建分支并进入新会话的“对话”视图。
...
该包同时是 DSH bundle 和浏览器客户端插件。bundle 层负责在 `web` profile 中启用插件；客户端注册 `conversation.view` 扩展，并从 DSH 获取会话存储、导航、fork 操作和 UI primitives。视图只投影会话元数据；会话持久化、修改以及当前“对话”或“轨迹”视图仍由 DSH 负责。
